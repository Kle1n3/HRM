      ...
      <el-row class="inline-info">
        <el-col>
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="employeeAvatar" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
...
import {
  ...
  saveEmployee,
  getEmployeePerseonalInfo,
  saveEmployeeJPersonalInfo,
} from "@/api/employees";
...
export default {
  ...
  methods: {
    async initData() {
      ...
      const res = await getEmployeePerseonalInfo(this.userid);
      this.userInfo = data;
      this.formData = res;
      if (data.staffPhoto.trim() !== "") {
        this.$refs.employeeAvatar.fileList = [
          { name: "Preview", url: data.staffPhoto, upload: true },
        ];
      }
      if (res.staffPhoto.trim() !== "") {
        this.$refs.employeePic.fileList = [
          { name: "Preview", url: data.staffPhoto, upload: true },
        ];
      }
    },
    async saveUser() {
        const status = this.$refs.employeeAvatar.fileList.some(item=>item.upload)
        if(!status) return this.$message.warning('is uploading')
        this.userInfo.staffPhoto = this.$refs.employeeAvatar.fileList[0].url
      await saveEmployee(this.userInfo);
      this.$message.success("更新成功");
    },
    async savePersonal() {
        const status = this.$refs.employeePic.fileList.some(item=>item.upload)
        if(!status) return this.$message.warning('is uploading')
        this.formData.staffPhoto = this.$refs.employeePic.fileList[0].url
      await saveEmployeeJPersonalInfo(this.formData);
      this.$message.success("更新成功");
    },
  },
};
