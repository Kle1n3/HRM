    ...
    <div class="app-container">
      <el-card>
        ...
        <el-row  type="flex" justify="end">
          <el-button v-print="'#box'" size="small" type="primary">打印</el-button>
   </el-row>
          <!-- 个人详情 -->
        <div id="box" v-if="type === 'personal'">
          <h2 class="centInfo">员工信息表</h2>
          <table cellspacing="0" border width="100%" class="tableList">
            <tr class="title">
...
<script>
import { getEmployeePerseonalInfo, getEmployeeJobInfo } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getEmployeePerseonalInfo() : this.getEmployeeJobInfo()
  },
  // 组件更新
  methods: {
    async getEmployeePerseonalInfo() {
      this.formData = await getEmployeePerseonalInfo(this.userId) // 获取个人基本信息
    },
    async getEmployeeJobInfo() {
      const userInfo = await getuserBaseInfoApi(this.userId)
      const jobInfo = await getEmployeeJobInfo(this.userId) // 获取个人基本信息
